...
func (h *CommentHandler) ListPostComments(c *gin.Context) {
	...
	threads, total, page, limit, err := h.commentService.ListByPost(
		c.Request.Context(), postID, page, limit,
	)
	if err != nil {
		h.respondCommentError(c, err, "list post comments")
		return
	}
	items := make([]dto.CommentData, 0, len(threads))
	for i := range threads {
		items = append(items, dto.NewCommentData(&threads[i].Comment, threads[i].Replies))
	}
...
func (h *CommentHandler) CreateComment(c *gin.Context) {
	...
	response.Ok(c, dto.NewCommentData(comment, nil))
}
