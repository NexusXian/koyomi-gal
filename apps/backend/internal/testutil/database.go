...
func TruncateTables(t *testing.T, db *gorm.DB) {
	t.Helper()
	err := db.Exec(
		"TRUNCATE TABLE notifications, user_preferences, image_assets, articles, banners, background_presets, feedback, resource_reports, post_favorites, comment_likes, post_likes, comments, posts, resource_links, resources, user_galgames, galgame_favorites, galgame_ratings, galgame_gallery_images, galgame_tags, galgame_aliases, galgames, tags, developers, user_roles, role_permissions, permissions, roles, users RESTART IDENTITY",
	).Error
	if err != nil {
