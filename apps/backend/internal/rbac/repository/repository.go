...
func (r *RBACRepository) TransactionWithDB(
	ctx context.Context,
	fn func(tx *RBACRepository, db *gorm.DB) error,
) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		return fn(&RBACRepository{db: tx}, tx)
	})
}
...
func (r *RBACRepository) FindRoleByIDForUpdate(ctx context.Context, roleID int64) (*model.Role, error) {
	var role model.Role
	err := r.db.WithContext(ctx).Clauses(clause.Locking{Strength: "UPDATE"}).First(&role, roleID).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, nil
	}
	if err != nil {
...
func (r *RBACRepository) FindRoleByCodeForUpdate(ctx context.Context, code string) (*model.Role, error) {
	var role model.Role
...
func (r *RBACRepository) CountUsersByRoleID(ctx context.Context, roleID int64) (int64, error) {
	var count int64
	err := r.db.WithContext(ctx).
		Model(&model.UserRole{}).
		Where("role_id = ?", roleID).
